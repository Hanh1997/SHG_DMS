//
//  ListNGItemViewController.swift
//  SH_SS
//
//  Created by Hung on 5/31/19.
//  Copyright © 2019 phạm Hưng. All rights reserved.
//

import UIKit

class ListNGItemViewController: UIViewController,UISearchBarDelegate,UIPickerViewDelegate,UIPickerViewDataSource,UITableViewDelegate,UITableViewDataSource {
    ...
    //Mở giao diện  lỗi sản phẩm
    func chooseNGItem() {
        let NGItem  : ReportErrorPCRoundViewController  = (UIStoryboard.init(name: "Main", bundle: Bundle.main).instantiateViewController(withIdentifier: "ReportErrorPCRound") as? ReportErrorPCRoundViewController)!
        self.navigationController?.pushViewController(NGItem, animated: true)
    }
    
    //Mở giao diện Lỗi các hạng mục tại điểm bán
    func chooseNGDebt() {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "CellOnListNG") as? ReportResultTableViewCell else
        {
    ...
    func getListNGItem(ngtype:String)
    {
        ...
        // create the request
        let url = URL(string: "http://appapi.sunhouse.com.vn/api/DMS/GetListDMSNGItem?fromdate=\(udate)&todate=\(udate)&username=\(userLogin)&ngtype=\(ngtype)&cmp_wwn=\(cmp_wwn)")!
        var request = URLRequest(url: url)
        request.httpMethod = "GET"
        request.setValue("Basic \(base64LoginData)", forHTTPHeaderField: "Authorization")
...
class NGItem {
    ...
    init(_ dictionary: [String: Any]) {
        ...
        self.NGType = dictionary["NGType"] as? String ?? ""
        self.NGDescription = dictionary["NGDescription"] as? String ?? ""
    }
}
