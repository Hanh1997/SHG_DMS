...
//  SH_SS
//
//  Created by phạm Hưng on 4/24/19.
...
class CheckOutViewController: UIViewController,CLLocationManagerDelegate ,UIImagePickerControllerDelegate ,UINavigationControllerDelegate,
URLSessionDelegate, URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    @IBOutlet var btnCheckOut: UIView!
    ...
    override func viewDidLoad() {
        ...
        txtCmp_name.text = UserDefaults.standard.string(forKey: "cmp_name") ?? ""
        ...
        ceo.reverseGeocodeLocation(loc, completionHandler:
            {(placemarks, error) in
                ...
                if pm.count > 0 {
                    ...
                    self.lbLocation.text = addressString
                    print(addressString)
              }
        })

    }
    @IBAction func func_camera(_ sender: Any) {
        func_camera()
    }
    
    @IBAction func func_showMap(_ sender: Any) {
        PassData()
    }
    ...
    @IBAction func CheckOutAction(_ sender: Any) {
        ...
        else
        {
            DispatchQueue.global(qos: .userInitiated).async {
                DispatchQueue.main.async {
                    // create the alert
                    let alert = UIAlertController(title: "Thông  báo", message: "Bạn có đồng ý check Out không?", preferredStyle: UIAlertController.Style.alert)
    ...
    //Check Out
    func checkOut(_cmp : String,_lat : Double, _Long : Double,_Distance:Double,_pathImg : String)
    {
        //self.addSavingPhotoView()
        ...
        let urlString = "http://appapi.sunhouse.com.vn/api/DMS/SaveCheckInData?cmp_wwn=\(_cmp)&latitude=\(_lat)&longitude=\(_Long)&username=\(userName)&distance=\(_Distance)&sysnote=appIos&imagepath=\(_pathImg)&&actioncode=CheckOut"
            ...
            do {
                ...
                if let chek = json?["ResponseStatus"] as? String {
                    ...
                    if(chek == "OK")
                    {
                            DispatchQueue.main.async {
                                // create the alert
                                let alert = UIAlertController(title: "Thông báo", message: "Check out thành công", preferredStyle: UIAlertController.Style.alert)
        ...
        URLSession.shared.uploadTask(with: urlRequest, from: data, completionHandler: { responseData, response, error in
            ...
            do {
                ...
                if ResponseStatus == "OK"
                {
                    ...
                    self.checkOut(_cmp: self.cmp_wwn, _lat: self.userlatitude, _Long: self.userlongitude, _Distance:self.distanceResult , _pathImg: self.imagrpath)
                }
